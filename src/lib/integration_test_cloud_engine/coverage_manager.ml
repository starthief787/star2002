  ...
  let download_coverage_data_from_nodes t ~nodes =
    ...
    let coverage_files =
      ...
      |> Malleable_error.List.fold ~init:[] ~f:(fun acc (_id, pod) ->
             ...
             let%bind () =
               Node.run_in_container ~cmd:[ "pkill"; "mina" ] pod >>| ignore
             in
             [%log' info t.logger] "Mina process in '$pod' killed."
               ~metadata:[ ("pod", `String pod_id) ] ;
             let%bind files_in_pwd = Node.list_files pod "/root/" in
             [%log' info t.logger] "Listing files in pwd '$pod' '$list'"
               ~metadata:
                 [ ("pod", `String pod_id)
                 ; ("list", `String (String.concat ~sep:"," files_in_pwd))
                 ] ;
             let%bind files_in_root = Node.list_files pod root in
             [%log' info t.logger] "Listing files in  '$pod' '$list'"
               ~metadata:
                 [ ("pod", `String pod_id)
                 ; ("list", `String (String.concat ~sep:"," files_in_root))
                 ] ;
    ...
    in
    [%log' info t.logger] "Test coverage data generated" ;
    coverage_files
end
